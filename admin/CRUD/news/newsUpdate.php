...
if(isset($_POST['updateBtn'])){
    ...
    $sql = "SELECT news_cat.category_name, news.news_title, news.news_link, news.news_description, news.news_date, news.is_active, news.news_category_category_id 
    ...
    $newsActive = $_POST['inputNewsActiveEdit'];

    $updateSQL = "UPDATE db_science_university_news 
    SET news_title=?, news_link=?, news_description=?, news_date=?, news_category_category_id=?, is_active=?
    WHERE id='$id'";
    $result = $conn->prepare($updateSQL);
    $runQuery = $result->execute([$news_title, $news_link, $news_description, $news_date, $news_category, $newsActive]);
    if($runQuery){
