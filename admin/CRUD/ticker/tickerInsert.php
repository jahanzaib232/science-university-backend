...
if(isset($_POST['submitBtn'])){
    ...
    $sql = "INSERT INTO db_science_university_ticker (icon_image, character_, character_before_number, number_, data_target, inc_or_decr, description_, is_active, db_science_university_users_id) 
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
    $result = $conn->prepare($sql);
    $runQuery = $result->execute([$ticker_icon, $ticker_character, $ticker_character_before_num, $ticker_number, $data_target, $ticker_counter, $ticker_description, $ticker_active, $userID]);
    if($runQuery){     
